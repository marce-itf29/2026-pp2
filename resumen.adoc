...
A[Diagrama de Flujo de Datos - DFD] --> B[Definición]
A --> C[Componentes]
A --> D[Reglas]
A --> E[Niveles]
A --> F[Almacenes de datos]

B --> B1[Herramienta de modelado]
B --> B2[Gráfica intuitiva]
B --> B3[Descomposición sucesiva]

C --> C1[Proceso - Círculo]
C --> C2[Flujo - Flecha]
C --> C3[Almacén - Líneas paralelas]
C --> C4[Terminador - Rectángulo]

C1 --> C1A[Verbo + Sustantivo]
C1 --> C1B[Numeración por nivel]

C2 --> C2A[Datos en movimiento]
C2 --> C2B[Debe llevar leyenda]

C3 --> C3A[Datos en reposo]
C3 --> C3B[Flujos de actualización]
C3 --> C3C[Flujos de consulta]

C4 --> C4A[Agente externo]
C4 --> C4B[Puede repetirse]
...
D5 --> D5A[Evitar sumideros infinitos]
D5 --> D5B[Evitar generación espontánea]
D5 --> D5C[Etiquetar todo]

E --> E1[Contexto - Sistema completo]
E --> E2[Nivel 1 - Procesos principales]
E --> E3[Nivel 2 - Detalle de cada proceso]

F --> F1[Identificar procesos]
F --> F2[Determinar datos necesarios]
F --> F3[Agrupar por relación]
F --> F4[Nombrar en plural]
----